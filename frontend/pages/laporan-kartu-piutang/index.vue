            ...
            <table class="table table-bordered table-sm">
              ...
              <tbody>
                <tr v-for="(rinc) in data.piutang" :key="rinc.id" class="text-center">
                  <td>{{ $moment(rinc.tanggal).format('DD-MM-YYYY') }}</td>
                  <td>
                    {{ rinc.keterangan }}
                  </td>
                  <td>
                  ...
                  <td>
                    {{ formatRupiah(rinc.mutasi.debit, 'Rp. ') }}
                  </td>
                  <td>
                    {{ formatRupiah(rinc.mutasi.kredit, 'Rp. ') }}
                  </td>
                  <td>
                    {{ formatRupiah(rinc.saldo.debit, 'Rp. ') }}
                  </td>
                  <td>
                    {{ formatRupiah(rinc.saldo.kredit, 'Rp. ') }}
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  methods: {
    ...
    fetchKeuanganRincian () {
      ...
      /** down here is the query if the paid one will not be showed */
      ...
      // `
      const query = `query getLaporanPiutangPerAngkatanAndOrJurusan($id_angkatan: Int!, $id_jurusan: uuid!, $waktu_pembayaran: date!) {
                      mahasiswa(where: {id_angkatan: {_eq: $id_angkatan}, kode_jurusan: {_eq: $id_jurusan}, 
                        keuangan_pembayaran_mahasiswas: {rincian: {is_lunas: {_eq: false},
                          _and: {waktu_pembayaran: {_lt: $waktu_pembayaran}}}}}, order_by: {prodi: {kode: asc}}) {
                        angkatan {
                          name
                        }
                        kelas_mahasiswa {
                        ...
                        id
                        keuangan_pembayaran_mahasiswas {
                          rincian(order_by: {id: asc}, where: {waktu_pembayaran: {_lt: $waktu_pembayaran}}) {
                            waktu_pembayaran
                            nama_pembayaran
                            jumlah_pembayaran
                            is_lunas
      ...
      this.mahasiswa.forEach((mhs) => {
        ...
        mhs.keuangan_pembayaran_mahasiswas[0].rincian.forEach((data) => {
          ...
          table.push({
            ...
            ref: '',
            mutasi: {
              debit: (!data.is_lunas) ? data.jumlah_pembayaran : '-',
              kredit: (data.is_lunas) ? data.jumlah_pembayaran : '-'
            },
            saldo: {
              debit: (debit >= 0) ? debit : '',
              kredit: (debit <= 0) ? Math.abs(debit) : ''
            }
          })
        })
