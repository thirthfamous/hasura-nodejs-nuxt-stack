      ...
      <table class="table table-bordered table-sm table-sm">
        ...
        <tbody>
          <tr v-for="(data, index) in mahasiswa" :key="data.id" class="text-center">
            ...
            <td>
              {{ (data.prodi !== null) ? data.prodi.nama : 'Jurusan belum ditentukan' }}
            </td>
            <td>
              {{ (data.nim) ? data.nim : 'NIM belum ditentukan' }}
            </td>
            <td>
              {{ (data.angkatan !== null) ? data.angkatan.name : 'Angkatan belum ditentukan' }}
            </td>
            <td>
              <a class="btn btn-info" @click="detailMahasiswa(data.id)"> Detail </a>
            </td>
          </tr>
        </tbody>
      </table>
...
export default {
  ...
  methods: {
    ...
    detailMahasiswa (nim) {
      this.$router.push({ path: `/spp-dpp/${nim}` })
    },
  ...
  mahasiswa(limit: $limit, offset: $offset, order_by: {id: desc}, 
    where: {nim: {_eq: $nim}, id_angkatan: {_eq: $id_angkatan}, kode_jurusan: {_eq: $id_prodi}}) {
    id
    nama
    email
    nim
  ...
  mahasiswa_aggregate(where: {nim: {_eq: $nim}, kode_jurusan: {_eq: $id_prodi}}) {
    aggregate {
      count
    }
  }
