    ...
    <div class="col-md-12 bg-white">
      ...
      <b-container fluid>
        <b-row class="my-1">
          ...
          <b-col sm="9">
            <v-select v-model="jurnal.kode_akun" label="name"  :options="akun_options" />
        ...
        <b-row class="my-1">
          ...
          <b-col sm="9">
            <b-form-datepicker v-model="jurnal.tanggal" size="sm" />
          </b-col>
        </b-row>
        <b-row class="my-1">
          <b-col sm="3">
          ...
          <div class="row">
            <div class="col-md-12">
              ...
              <div style="margin: 10px 0px;">
                <a class="btn btn-success" @click="showConfirmUpdateMasterSubledger">Tambah</a>
              </div>
            </div>
          </div>
        </b-modal> -->
...
export default {
  ...
  methods: {
    showConfirmUpdateMasterSubledger () {
      ...
      const kode = this.jurnal.kode_akun.kode
    ...
    async save () {
      const query = `mutation UpdateJurnal($id: Int!, $debit: Int!, $keterangan: String!, $kredit: Int!, $ref: String!, $tanggal: date!, $kode_akun: String!) {
                    update_akuntansi_jurnal_by_pk(pk_columns: {id: $id}, _set: {debit: $debit, keterangan: $keterangan, kredit: $kredit, ref: $ref, tanggal: $tanggal, kode_akun: $kode_akun}) {
                        keterangan
                        kode_akun
                        kredit
                        ref
      ...
      const kode_akun = (typeof this.jurnal.kode_akun === 'object') ? this.jurnal.kode_akun.kode : this.jurnal.kode_akun
      const tanggal = (this.jurnal.tanggal) ? this.jurnal.tanggal : null
      const keterangan = (this.jurnal.keterangan) ? this.jurnal.keterangan : null
      const ref = (this.jurnal.ref) ? this.jurnal.ref : null
      const kredit = this.jurnal.kredit
      const debit = this.jurnal.debit
      ...
      await fetch(process.env.gql, {
        ...
        body: JSON.stringify({
          query,
          variables: { id, kode_akun, tanggal, keterangan, ref, kredit, debit }
        })
      })
        .then(res => res.json())
        .then((gql) => {
    ...
    fetchData () {
      const query = `query FetchForUpdateJurnal($id: Int!) {
                        akuntansi_jurnal_by_pk(id: $id) {
                            debit
                            id
                            keterangan
                            kode_akun
                            ...
                            nama
                            saldo_normal
                            akuntansi_master_subledgers {
                                akuntansi_transaksi_subledgers {
                                debit
                                id
                                keterangan
                                kredit
      ...
      fetch(process.env.gql, {
        ...
        .then((gql) => {
          ...
          this.jurnal = gql.data.akuntansi_jurnal_by_pk
        }).catch((error) => {
          this.errorMessage = error
          console.error('There was an error!', error)
        })
